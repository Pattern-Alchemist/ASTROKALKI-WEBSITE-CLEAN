...
import { sendEmail, dripCampaigns } from '@/lib/email/config';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const course = dripCampaigns.emailCourse;
    await sendEmail({
      to: email,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const enrollments = await db.emailCourseEnrollment.findMany({
      where: {
        stage: { lt: course.duration }, // Not yet complete
      },
    });
      ...
      try {
        ...
        await db.emailCourseEnrollment.update({
          ...
          data: {
            stage: day,
            lastSentAt: new Date(),
            completedAt: day === course.duration ? new Date() : null,
          },
        });
